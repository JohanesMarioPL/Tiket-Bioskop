...
@section('content')
<div class="bg-[#FAF3E0] overflow-x-hidden pb-20">
    <div class="px-4 md:px-10 py-0">
        <span class="font-baloo text-[#4B3935] text-2xl md:text-3xl mt-8 md:mt-10 flex flex-wrap gap-1 justify-center items-center mb-8 md:mb-10 text-center font-bold">
            Hey There! Wanna watch a movie?
        </span>
        
        <div class="w-full h-[250px] md:h-[400px] bg-[#4B3935] rounded-3xl mb-10 overflow-hidden relative shadow-md">
        ...
        <div class="flex flex-row justify-between items-center grow-0 px-2 md:px-10 mt-8 md:mt-0 mb-6">
            <span class="font-baloo text-xl md:text-2xl font-bold text-[#4B3935]">Featured Movies</span>
            <a href="{{ url('/movies') }}" class="bg-[#4B3935] text-[#FAF3E0] px-5 py-2 rounded-3xl text-sm font-bold hover:bg-[#5c4944] transition-colors">
                See all
            </a>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 px-2 md:px-10 mb-12">
            @forelse(($featuredMovies ?? [])->take(4) as $movie)
                ...
                @endphp
                <a href="{{ route('movies.show', $movie->id) }}" class="bg-white rounded-2xl shadow-sm border border-[#C8C2BC]/40 overflow-hidden hover:-translate-y-1 transition-transform group flex flex-col h-full">
                    ...
                    <div class="p-4 flex flex-col flex-1">
                        <h3 class="font-bold text-[#4B3935] text-base md:text-lg mb-1 min-h-[3rem] line-clamp-2 transition-colors leading-tight">{{ $movie->title }}</h3>
        ...
        <div class="mx-2 md:mx-10 mt-8 md:mt-10 mb-12">
            @if(isset($randomBanners[0]))
                <div class="w-full h-[250px] md:h-[350px] bg-[#4B3935] rounded-3xl overflow-hidden relative group shadow-md">
                    <img src="{{ $randomBanners[0]->getRawOriginal('poster_url') ? $randomBanners[0]->poster_url : 'https://via.placeholder.com/1200x500' }}" class="w-full h-full object-cover opacity-50 group-hover:scale-105 group-hover:opacity-60 transition-all duration-700">
                    <div class="absolute bottom-0 left-0 p-6 md:p-10 w-full bg-gradient-to-t from-black/80 to-transparent">
                        ...
                        <a href="{{ route('movies.show', $randomBanners[0]->id) }}" class="inline-block bg-white text-[#4B3935] px-6 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-slate-100 transition-colors">
                            Get Tickets
                        </a>
                    </div>
                </div>
        ...
        <div class="flex flex-row justify-between items-center grow-0 mt-8 md:mt-12 mb-6 md:mb-10 px-2 md:px-10">
            <span class="font-baloo text-xl md:text-2xl font-bold text-[#4B3935]">Top Movies</span>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 px-2 md:px-10 mb-12 max-w-full">
            @php
                ...
                $rankColors = ["bg-[#FFD700]", "bg-[#C0C0C0]", "bg-[#CD7F32]", "bg-[#4B3935]"];
            @endphp
            ...
            @forelse(($topMovies ?? [])->take(4) as $index => $movie)
                ...
                <div class="relative w-full hover:-translate-y-2 transition-transform duration-300">
                    ...
                    <a href="{{ route('movies.show', $movie->id) }}" class="bg-white rounded-2xl shadow-sm border border-[#C8C2BC]/40 overflow-hidden h-full flex flex-col group">
                        <div class="w-full aspect-[3/4] bg-slate-200 overflow-hidden">
                            <img src="{{ $movie->poster_url }}" alt="{{ $movie->title }}" class="w-full h-full object-cover">
                        </div>
                        <div class="p-4 flex flex-col flex-1">
                            <h3 class="font-bold text-[#4B3935] text-base md:text-lg mb-1 min-h-[3rem] line-clamp-2 leading-tight transition-colors">{{ $movie->title }}</h3>
        ...
        <div class="mx-2 md:mx-10 mt-10 md:mt-20 mb-12"> 
            @if(isset($randomBanners[1]))
                 <div class="w-full h-[250px] md:h-[350px] bg-[#4B3935] rounded-3xl overflow-hidden relative group shadow-md">
                    <img src="{{ $randomBanners[1]->getRawOriginal('poster_url') ? $randomBanners[1]->poster_url : 'https://via.placeholder.com/1200x500' }}" class="w-full h-full object-cover opacity-50 group-hover:scale-105 group-hover:opacity-60 transition-all duration-700">
                    <div class="absolute bottom-0 left-0 p-6 md:p-10 w-full bg-gradient-to-t from-black/80 to-transparent">
                        <span class="bg-[#f5cf14] text-white px-3 py-1 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-wider mb-3 inline-block">Must Watch</span>
                        <h2 class="text-2xl md:text-4xl font-extrabold text-white mb-3">{{ $randomBanners[1]->title }}</h2>
                        <a href="{{ route('movies.show', $randomBanners[1]->id) }}" class="inline-block bg-white text-[#4B3935] px-6 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-slate-100 transition-colors">
                            Get Tickets
                        </a>
                    </div>
                </div>
        ...
        <div class="flex flex-row justify-between items-center grow-0 px-2 md:px-10 mt-8 md:mt-10 mb-6">
            <span class="font-baloo text-xl md:text-2xl font-bold text-[#4B3935]">Upcoming This Week</span>
            <a href="{{ url('/movies') }}" class="bg-[#4B3935] text-[#FAF3E0] px-5 py-2 rounded-3xl text-sm font-bold hover:bg-[#5c4944] transition-colors shadow-sm">
                See all
            </a>
        </div>
